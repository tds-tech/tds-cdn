/**
 * TDS AI Visibility Scanner — Webflow Embed v1.1.1
...
(function () {
  ...
  var GRADES = {
    'AI-Ready': { color: '#0d9488', glow: 'rgba(13,148,136,.28)', label: 'AI★' },
    'A':  { color: '#0d9488', glow: 'rgba(13,148,136,.28)', label: 'A' },
    'B':  { color: '#2563eb', glow: 'rgba(37,99,235,.28)',  label: 'B' },
    'C':  { color: '#d97706', glow: 'rgba(217,119,6,.28)',  label: 'C' },
    'D':  { color: '#dc2626', glow: 'rgba(220,38,38,.28)',  label: 'D' },
    'F':  { color: '#991b1b', glow: 'rgba(153,27,27,.28)',  label: 'F' },
    'Not Found': { color: '#6b7280', glow: 'rgba(107,114,128,.2)', label: '?' }
  };
  ...
  var CSS = [
    ...
    '}',
    '#tds-ai-scanner{max-width:680px;margin:0 auto;padding:16px;}',
    '@media(min-width:640px){#tds-ai-scanner{padding:40px 24px;}}',
    ...
    '  letter-spacing:.01em;',
    '}',
    '.tds-btn:hover:not(:disabled){transform:translateY(-1px);box-shadow:0 6px 22px rgba(37,99,235,.45);}',
    '.tds-btn:active:not(:disabled){transform:translateY(0);}',
    '.tds-btn:disabled{opacity:.65;cursor:not-allowed;transform:none;}',
    ...
    '.tds-step--active .tds-step-dot{',
    '  border-color:#2563eb;background:#eff6ff;',
    '  animation:tds-spin .9s linear infinite;',
    '}',
    '.tds-step--done{color:#0d9488;}',
    '.tds-step--done .tds-step-dot{border-color:#0d9488;background:#f0fdfa;color:#0d9488;}',
    '.tds-prog-track{height:5px;background:#f1f5f9;border-radius:99px;overflow:hidden;margin-bottom:6px;}',
    '.tds-prog-fill{',
    ...
    /* Grade badge */
    '.tds-grade-hero{display:flex;align-items:center;gap:18px;margin-bottom:20px;}',
    '.tds-grade-badge{',
    ...
    '.tds-score-num{font-size:22px;font-weight:800;margin:0 0 6px;color:var(--tds-color,#0f172a);}',
    '.tds-score-bar{height:4px;background:#f1f5f9;border-radius:99px;overflow:hidden;margin-bottom:8px;}',
    ...
    '  padding:13px 17px;font-size:14px;color:#374151;',
    ...
    '}',
    '.tds-cat-group{margin-bottom:18px;}',
    '.tds-cat-title{',
    ...
    /* CTA block */
    '.tds-cta-block{',
    '  background:linear-gradient(135deg,#1e40af,#2563eb 50%,#0d9488);',
    ...
    '.tds-cta-block p{font-size:14px;color:rgba(255,255,255,.82);margin:0 0 18px;line-height:1.6;}',
    '.tds-cta-btn{',
    '  display:inline-flex;align-items:center;gap:6px;',
    '  padding:13px 26px;background:#fff;color:#1d4ed8;',
  ...
  function svgGlobe() { return '&#127760;'; }
  function svgShare() { return '&#128279;'; }

  function statusSvg(s) {
  ...
  function renderChecklist(checks, blurred) {
    ...
    var catOrder = ['ai_presence', 'local_seo', 'structured_data', 'content_signals', 'technical'];
    var keys = catOrder.filter(function (k) { return groups[k]; });
    Object.keys(groups).forEach(function (k) { if (keys.indexOf(k) === -1) keys.push(k); });

    return keys.map(function (cat) {
      var label = CATEGORY_LABELS[cat] || cat;
      var items = groups[cat].map(function (c) {
        var cls = blurred ? 'tds-check-item tds-blur' : 'tds-check-item';
        return [
          '<li class="' + cls + '">',
          statusSvg(c.status),
          '<div>',
  ...
  function renderStage2Polling(root, domain, scanId) {
    var stepDots = SCAN_STEPS.map(function (s, i) {
      return '<li class="tds-step' + (i === 0 ? ' tds-step--active' : '') + '">' +
        '<span class="tds-step-dot">' + (i === 0 ? '' : '') + '</span>' + esc(s) +
        '</li>';
    }).join('');
