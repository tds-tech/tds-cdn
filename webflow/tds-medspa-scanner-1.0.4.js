/**
 * TDS Med Spa AI Visibility Scanner — Webflow Embed v1.0.4 (fork of dental v1.8.0)
 ...
 *
 * v1.8.0 — "Heads up: what we found" alert card (Ramiro + Gabriel, 2026-07-14):
 *   • New .tds-gaps card in the FULL report (post-gate + ?r= page), between
 *     competitors and the CTA: top-5 failed checks as fixed patient-language
 *     alert lines (diagnostics.gaps, computed engine-side, $0, no LLM).
 *   • Alert framing on purpose: the report names the problem, the booked
 *     call sells the fix. Teaser stays narrative+competitors (the hook);
 *     the gate unlocks the found-problems list.
 *   • Narrative is now 2 short paragraphs + 1 hope line (engine change) —
 *     no fix advice in prose anymore.
 * v1.7.2 — Same ring fix for the card itself (focusStage falls back to
 *   .tds-card when a stage has no heading, e.g. the error card).
 * v1.7.1 — No focus ring on the programmatic stage-focus target (the practice
 *   name drew a blue outline box on ?r= page loads). Standard skip-target
 *   pattern: non-interactive [tabindex="-1"] targets show no outline; real
 *   keyboard focus on buttons/inputs keeps its ring.
 * v1.7.0 — Dentist-first report (Gabriel rounds 5+6, 2026-07-09/14, ClickUp 86e1j4d5u):
 *   • Title FIRST: grade card renders before the written summary in both the
 *     teaser and the full report ("why is this text above the title?" — the
 *     grade hero's full-bleed negative margins overlapped whatever sat above it).
 *   • Technical checklist HIDDEN: the report is narrative + competitors + CTA.
 *     The 16 checks live in a collapsed "technical details" block for the
 *     reader's web team ("I wouldn't even show it, nobody would understand").
 *     Teaser no longer shows any checks or check counts.
 *   • Practice names decode HTML entities ("Siouxland Oral &amp; Maxillofacial
 *     Surgery" → "&") — engine now stores decoded, widget decodes defensively
 *     for cached rows.
 *   • Competitors block: crisper card edge + breathing room below (Jul 9 ask).
 *   • CTA (book a call) moved above the technical block: read → act.
...
(function () {
  ...
  var N8N_TEASER = 'https://ramsautomation.app.n8n.cloud/webhook/medspa-scanner-teaser';
  var N8N_POLL   = 'https://ramsautomation.app.n8n.cloud/webhook/ai-scanner-poll';
  var N8N_GATE   = 'https://ramsautomation.app.n8n.cloud/webhook/medspa-scanner-gate';
  ...
  var AI_SEO_URL = 'https://www.thedigitalsmile.com/med-spa-ai-visibility-checker';
  ...
  var BOOK_A_CALL_URL = 'https://thedigitalsmile-seo.com/booking?utm_source=ai-scanner-medspa&utm_medium=report&utm_campaign=aivc-report';
  ...
  var SCAN_STEPS = [
    'Querying ChatGPT &amp; Perplexity for your med spa…',
  ...
  var CSS = [
    ...
    /* Competitors: who AI recommends instead (familiar names make it real).
       Gabriel Jul 9: needs a real card edge + breathing room below. */
    '.tds-competitors{background:#fff7ed;border:1.5px solid #fdba74;border-radius:12px;padding:16px 20px;margin:0 0 26px;box-shadow:0 1px 3px rgba(154,52,18,.06);}',
    ...
    /* "Heads up" alert card — what the scan found, patient language. Soft
       red so it reads as a warning next to the orange competitors card. */
    '.tds-gaps{background:#fef2f2;border:1.5px solid #fca5a5;border-radius:12px;padding:16px 20px;margin:0 0 26px;box-shadow:0 1px 3px rgba(153,27,27,.06);text-align:left;}',
    '.tds-gaps-title{font-size:12px;font-weight:800;color:#b91c1c;text-transform:uppercase;letter-spacing:.06em;margin:0 0 8px;}',
    '.tds-gaps ul{list-style:none;padding:0;margin:0;}',
    '.tds-gaps li{font-size:14.5px;color:#334155;line-height:1.6;padding:8px 0;border-bottom:1px solid #fee2e2;}',
    '.tds-gaps li:last-child{border-bottom:none;padding-bottom:2px;}',
    '.tds-gaps li strong{color:#0f172a;font-weight:700;}',
    ...
    /* Programmatic stage-focus target (focusStage): announce to SR without a
       visible ring — it is not interactive. Real controls keep :focus-visible. */
    '.tds-card [tabindex="-1"]:focus,.tds-card [tabindex="-1"]:focus-visible,.tds-card[tabindex="-1"]:focus,.tds-card[tabindex="-1"]:focus-visible{outline:none;}',

    /* Technical details — collapsed by default. The report a dentist reads is
       narrative + competitors + CTA; the checklist is for their web team. */
    '.tds-tech{margin-top:20px;border-top:1px solid #f1f5f9;padding-top:6px;}',
    '.tds-tech summary{display:flex;align-items:center;gap:8px;flex-wrap:wrap;cursor:pointer;',
    '  font-size:13px;font-weight:700;color:#64748b;padding:10px 2px;min-height:44px;list-style:none;user-select:none;}',
    '.tds-tech summary::-webkit-details-marker{display:none;}',
    '.tds-tech summary:hover{color:#0f172a;}',
    '.tds-tech-arrow{display:inline-block;transition:transform .18s ease-out;color:#94a3b8;}',
    '.tds-tech[open] .tds-tech-arrow{transform:rotate(90deg);}',
    '.tds-tech-note{font-weight:500;font-size:12px;color:#94a3b8;}',
    '.tds-tech-body{padding-top:10px;}',
    '@media(prefers-reduced-motion:reduce){.tds-tech-arrow{transition:none;}}',
  ...
  function renderCompetitors(comps, city) {
    ...
    return [
      ...
      '<p class="tds-comp-title">Who clients see instead of you</p>',
      '<p class="tds-comp-sub">When people ask AI for a med spa' + (city ? ' in ' + esc(city) : ' near you') + ', these names come up:</p>',
      '<ul>' + items + '</ul>',
      '</div>'
    ].join('');
  }

  // Deterministic alert list from the engine (diagnostics.gaps): what the
  // scan found, in patient language. Alert framing on purpose — the report
  // names the problem, the booked call sells the fix. Full report only
  // (post-gate + shared ?r= page); the teaser keeps narrative+competitors
  // as the hook and the gate unlocks this list.
  function renderGaps(gaps) {
    if (!gaps || !gaps.length) return '';
    var items = gaps.map(function (g) {
      return '<li><strong>' + esc(g.title || '') + '</strong> ' + esc(g.detail || '') + '</li>';
    }).join('');
    return [
      '<div class="tds-gaps">',
      '<p class="tds-gaps-title">Heads up: what we found on your site</p>',
      '<ul>' + items + '</ul>',
      '</div>'
    ].join('');
  }
  ...
  // Practice names arrive entity-encoded from some sites ("Oral &amp; Maxillofacial").
  // Decode BEFORE esc() so the visible text shows the real "&". Textarea trick
  // never executes markup, so this is safe on untrusted strings.
  function decodeEntities(s) {
    if (s == null || String(s).indexOf('&') === -1) return s;
  ...
  function renderStage1(root) {
    root.innerHTML = [
      ...
      '<h2 class="tds-headline">Is your med spa invisible to AI?</h2>',
      '<p class="tds-sub">ChatGPT and Gemini are the new search engines for clients.',
      ' Run a free scan to see if your med spa shows up, and exactly what to fix if it doesn’t.</p>',
      '<div class="tds-pills">',
      ...
      '</div>',
      '<label class="tds-field-label" for="tds-d">Your med spa website</label>',
      '<div class="tds-input-wrap">',
      '<span class="tds-input-icon" aria-hidden="true">' + svgGlobe() + '</span>',
      '<input id="tds-d" class="tds-input" type="text" inputmode="url" placeholder="yourmedspa.com"',
    ...
    function submit() {
      ...
      if (!raw) {
        err.textContent = 'Please enter your med spa website.';
        flagError(input);
        return;
      }
  ...
  function renderStage2Teaser(root, domain, scanId, teaserData) {
    ...
    var name   = decodeEntities(teaserData.practice_name) || domain;

    // Dentist-first teaser (Gabriel): title/grade card FIRST, then the written
    // summary, then who AI recommends instead, then the unlock. The technical
    // checklist is gone from the teaser entirely — nobody non-technical
    // understood it, and the narrative is the hook.
    var gateTitle = checks.length ? ('Unlock your full ' + checks.length + '-point report') : 'Unlock your full report';
    var gateSub = 'Get the complete written report sent to your inbox as a PDF.';
    ...
    var city        = decodeEntities(teaserData.city || (teaserData.diagnostics && teaserData.diagnostics.city) || null);
    ...
    root.innerHTML = [
      ...
      ' placeholder="you@yourmedspa.com" aria-label="Work email" aria-describedby="tds-e2" aria-required="true" required>',
      '</div>',
      '<div class="tds-input-wrap">',
  ...
  function renderStage3Report(root, data) {
    ...
    var name      = decodeEntities(data.practice_name) || data.domain || 'Your med spa';
    var narrative = data.narrative || '';
    var competitors = data.competitors || (data.diagnostics && data.diagnostics.competitors) || [];
    var gaps      = data.gaps || (data.diagnostics && data.diagnostics.gaps) || [];
    var city      = decodeEntities(data.city || (data.diagnostics && data.diagnostics.city) || null);
    ...
    // Dentist-first report (Gabriel): title/grade → written report → who AI
    // recommends instead → book a call. The 16-check breakdown is reference
    // material for their web team — collapsed by default, out of the story.
    var techBlock = checks.length ? [
      '<details class="tds-tech">',
      '<summary><span class="tds-tech-arrow" aria-hidden="true">&#9656;</span>See the technical details',
      '<span class="tds-tech-note">the ' + checks.length + ' point checklist for your web team</span></summary>',
      '<div class="tds-tech-body">' + renderChecklist(checks) + '</div>',
      '</details>'
    ].join('') : '';
  ...
  function renderPublicResult(root, slug) {
    ...
    post(N8N_RESULT, { slug: slug }, function (e, data) {
      ...
      renderStage3Report(root, {
        ...
        gaps: data.gaps || (data.diagnostics && data.diagnostics.gaps) || [],
